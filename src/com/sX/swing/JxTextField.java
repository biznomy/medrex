...
public class JxTextField extends JxComponent {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6865869410664358999L;
	private float arc;
	private float scale;
	private Shape shape;
	...
	protected void loadPopup(String str) {
		List items = model.getValues(str);
		hidePopup();
	...
	protected boolean isBindable() {
		if (model instanceof BindableListModel) {
			return true;
		}
		return false;
	}
		...
		if (isBindable()) {
			...
			popupItemList.addKeyListener(new KeyAdapter() {
				...
				public void keyReleased(KeyEvent e) {
					...
					if (e.getKeyCode() == MenuKeyEvent.VK_ENTER) {
						...
						if (obj != null) {
							JxTextField.this.document.setText(model
									.toString(obj));
							fireObjectChanged();
							hidePopup();
						}
			...
			popupItemList.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					Object obj = getSelectedItem();
					if (obj != null) {
						JxTextField.this.document.setText(model.toString(obj));
						fireObjectChanged();
						hidePopup();
					}
				}
	...
	protected void showOptions() {
		if (btnOptionsMenu != null) {
			...
			final BindableListModel bindableModel = (BindableListModel) model;
