...
public class FrameAssessmentsFloorsInfo extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 437882557204287127L;
	private JxTable table;
	private AssessmentsTableModalFloors refJTableModal;
	private boolean flag = false;

	public FrameAssessmentsFloorsInfo() {
		super();
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent evt) {
				Global.currentResidenceArgument = 0;
				FrameAssessmentsFloorsInfo.this.dispose();
			}
		});
		btnOk.setText("Ok");
		...
		lblTitle.setText("Assessments Information Floor Wise:");
		...
		refJTableModal = new AssessmentsTableModalFloors();
	...
	public void updateJTable(int type) {
		...
		try {
			switch (type) {
			// lbl1stFloorPendingValCol2
			case 0:
				records = MedrexClientManager.getInstance()
						.getFirstFloorPendingFormResidents();
				break;
			case 1:
				records = MedrexClientManager.getInstance()
		...
		while (it.hasNext()) {
			...
			int pendingCount = new Integer(pendingRecord[1].toString())
					.intValue();
			String floorType = pendingRecord[2].toString() + "";
...
class AssessmentsTableModalFloors extends AbstractTableModel {
	/**
	 * 
	 */
	private static final long serialVersionUID = -5190057334604066394L;

	private String[] columnNames = { "Resident", "Floor", "Total Pending From" };
	...
	public AssessmentsTableModalFloors(List<PendingCountBean> recordsList) {
	...
	public AssessmentsTableModalFloors() {
