...
public class PanelNoteCategoriesMain extends JPanel {
	...
	private NoteCategoriesTableModel tableModel;
	private JxComboBox cmbTemplateType;
	private PanelNoteCategoriesDialog panelNoteCategoriesDialog;
	private String strTemp;

	public PanelNoteCategoriesMain() {
		super();
		...
		lblHeading.setText("Notes Categories");
		...
		tableModel = new NoteCategoriesTableModel();
		table = new JxTable();
			...
			public void mouseClicked(final MouseEvent e) {
				if (e.getClickCount() == 2) {
					int id = (Integer) tableModel.getValueAt(table
							.getSelectedRow(), 2);
					Global.currentNoteCategoriesKey = id;
					if (id != 0) {
						panelNoteCategoriesDialog = new PanelNoteCategoriesDialog();
						SwingUtils.openInDialog(panelNoteCategoriesDialog);
						updateTable();
					}
				}
			}
		...
		addNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				Global.currentNoteCategoriesKey = 0;
				panelNoteCategoriesDialog = new PanelNoteCategoriesDialog();
				SwingUtils.openInDialog(panelNoteCategoriesDialog);
				updateTable();
			}
	...
	private void updateTable() {
		List<NotesCategories> list = null;
		try {
			list = MedrexClientManager.getInstance().getAllNotesCategories();
		} catch (Exception e) {
			e.printStackTrace();
		}
...
class NoteCategoriesTableModel extends AbstractTableModel {
	/**
	 * 
	 */
	private static final long serialVersionUID = -533624199875753240L;
	private List<NotesCategories> records;
	private String[] columnNames = { "Form Type", "Notes Category" };

	public NoteCategoriesTableModel() {
		this(new ArrayList<NotesCategories>());
	}

	public NoteCategoriesTableModel(List<NotesCategories> rec) {
		setRecords(rec);
	}
	...
	public void setRecords(List<NotesCategories> rec) {
		this.records = rec;
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		NotesCategories record = records.get(rowIndex);
		if (record != null) {
			switch (columnIndex) {
			case 0:
				return record.getFormType();
			case 1:
				return record.getNoteCategory();
			case 2:
