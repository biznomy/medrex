...
public class PanelTemplateMain extends JPanel {
	...
	private PanelTemplateDialog panelTemplateDialog;
	private String strTemp;

	public PanelTemplateMain() {
		super();
			...
			public void mouseClicked(final MouseEvent e) {
				if (e.getClickCount() == 2) {
					int id = (Integer) tableModel.getValueAt(table
							.getSelectedRow(), 2);
					Global.currentTemplateKey = id;
					if (id != 0) {
						panelTemplateDialog = new PanelTemplateDialog();
						SwingUtils.openInDialog(panelTemplateDialog);
						updateTable();
					}
				}
			}
			...
			public void actionPerformed(final ActionEvent e) {
				try {
					int id = (Integer) tableModel.getValueAt(table
							.getSelectedRow(), 2);
					if (id != 0) {
						MedrexClientManager.getInstance().deleteTemplate(id);
						updateTable();
					}
				} catch (Exception ex) {
				}
	...
	private void updateTable() {
		List<Templates> list = null;
		try {
			list = MedrexClientManager.getInstance().getAllTemplates();
		} catch (Exception e) {
			e.printStackTrace();
		}
...
class TemplateTableModel extends AbstractTableModel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8781373783949931113L;
	private List<Templates> records;
	private String[] columnNames = { "Template Type", "Template Name" };

	public TemplateTableModel() {
		this(new ArrayList<Templates>());
	}

	public TemplateTableModel(List<Templates> rec) {
		setRecords(rec);
	}
	...
	public void setRecords(List<Templates> rec) {
		this.records = rec;
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		Templates record = records.get(rowIndex);
		if (record != null) {
			switch (columnIndex) {
			case 0:
