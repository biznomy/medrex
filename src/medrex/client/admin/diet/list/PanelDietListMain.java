...
public class PanelDietListMain extends JPanel {
	...
	private DietListTableModel tableModel;
	private PanelDietListDialog panelDietListDialog;

	public PanelDietListMain() {
		super();
		...
		tableModel = new DietListTableModel();
		table = new JxTable();
			...
			public void mouseClicked(final MouseEvent e) {
				if (e.getClickCount() == 2) {
					int id = (Integer) tableModel.getValueAt(table
							.getSelectedRow(), 2);
					Global.currentDietListKey = id;
					if (id != 0) {
						panelDietListDialog = new PanelDietListDialog();
						SwingUtils.openInDialog(panelDietListDialog);
						updateTable();
					}
				}
			}
			...
			public void actionPerformed(final ActionEvent e) {
				try {
					int id = (Integer) tableModel.getValueAt(table
							.getSelectedRow(), 2);
					if (id != 0) {
						MedrexClientManager.getInstance().deleteDiet(id);
						updateTable();
					}
				} catch (Exception ex) {
				}
	...
	private void updateTable() {
		List<DietList> list = null;
		try {
			list = MedrexClientManager.getInstance().getAllDiets();
		} catch (Exception e) {
			e.printStackTrace();
		}
...
class DietListTableModel extends AbstractTableModel {
	/**
	 * 
	 */
	private static final long serialVersionUID = 2547169159767107144L;
	private List<DietList> records;
	private String[] columnNames = { "Name", "Category" };

	public DietListTableModel() {
		this(new ArrayList<DietList>());
	}

	public DietListTableModel(List<DietList> rec) {
		setRecords(rec);
	}
	...
	public void setRecords(List<DietList> rec) {
		this.records = rec;
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		DietList record = records.get(rowIndex);
		if (record != null) {
			switch (columnIndex) {
			case 0:
