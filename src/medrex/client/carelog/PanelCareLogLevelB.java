...
public class PanelCareLogLevelB extends JPanel implements LtcButtonListener {
	...
	public PanelCareLogLevelB() {
		super();
		Global.panelCareLogLevelB = this;
		...
		lblADL = new JLabel();
		lblADL.setFont(new Font("", Font.BOLD, 20));
		lblADL
...
//				toolTip.setVisible(true);
		...
		panel_7 = new JPanel();
		panel_7.setBounds(482, 20, 120, 54);
		panel.add(panel_7);
	...
	public void updateSelections() {
		...
		if (GlobalCareLog.currentCareLogSubOptionKey != 0) {
			CareLogOption clo = GlobalCareLog
					.getCareLogOptionWithNum(GlobalCareLog.currentCareLogSubOptionKey);
			...
			if (GlobalCareLog.currentCareLogKey != 0) {
				...
				int val = -1;
				switch (GlobalCareLog.currentCareLogSubOptionKey) {
				case 1:
					val = cl.getVal1();
					break;
				case 2:
	...
	public void doSave() {
		if (GlobalSecurity.isUpdatePriviledge(SubAppTypes.CARELOG)) {
			if (selected != -1) {
				...
				try {
					MedrexClientManager.getInstance().insertOrUpdateCareLog(cl);
				} catch (Exception e) {
