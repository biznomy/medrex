...
public class Unzipper {
	...
	public static void main(String[] args) {
		...
		String unZipFile = "C:/abc.zip";
		String unZipOutFolder = "C:/abc/";
	...
	public File recursiveUnzip(File inFile, File outFolder) {
		try {
			...
			while ((entry = in.getNextEntry()) != null) {
				...
				File newFile = new File(outFolder.getPath() + "/"
						+ entry.getName());
				...
				if (!entry.isDirectory()) {
					...
					if (entry.getName().toLowerCase().endsWith(".zip")) {
						String zipFilePath = outFolder.getPath() + "/"
								+ entry.getName();
						this.recursiveUnzip(new File(zipFilePath), new File(
								zipFilePath.substring(0,
