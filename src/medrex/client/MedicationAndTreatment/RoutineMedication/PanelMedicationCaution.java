...
public class PanelMedicationCaution extends JxPanel {

	/**
	 * 
	 */
	private static final long serialVersionUID = -4368226832589399948L;
	private JxToggleButton preparedButton;
	private JxToggleButton refuseButton;
	private JxToggleButton givenButton;
	...
	private MedicationView currentMedication = null;
	...
	public PanelMedicationCaution(
			FrameMedicationDesktop frameMedicationDesktop,
			MedicationView currentMedication) {
		super();
		...
		northPanel.setPreferredSize(new Dimension((int) (width / 1.3),
				(int) (height / 3.1)));
		...
		panel_2.setPreferredSize(new Dimension(120, 230));
		// panel_2.setPreferredSize(new Dimension(120, (int) (height/3)));
		...
		try {
			cautionName = MedrexClientManager.getInstance().getCautionaryName(
					frameMedicationDesktop.getCurrentMedication()
							.getMedicationId());
		...
		givenButton = new JxToggleButton();
		givenButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				doSave(1, Global.frameMedicationDashboard
		...
		refuseButton = new JxToggleButton();
		refuseButton.setText("REFUSED");
		refuseButton.setActionCommand("2");
		refuseButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				SwingUtils.openInDialog(new PanelMedicationDecline());
				doSave(2, PanelMedicationCaution.this.currentMedication);
			}
		});
		refuseButton.setBorder(new EtchedBorder(EtchedBorder.LOWERED));
		final FlowLayout flowLayout_2 = new FlowLayout();
		flowLayout_2.setVgap(10);
		refuseButton.setLayout(flowLayout_1);
		refuseButton.setFont(FONT);
		refuseButton.setBorderPainted(false);
		refuseButton.setArc(0.6f);
		refuseButton.setBorder(BORDER);
		refuseButton.setBackground(BACKGROUND);
		refuseButton.setForeground(FOREGROUND);
		refuseButton.setRolloverEnabled(true);
		refuseButton.setHorizontalAlignment(SwingConstants.CENTER);
		bgMedicationGroup.add(refuseButton);
		refuseButton.setPreferredSize(new Dimension(250, 80));
		refuseButton.setBounds(346, 269, 192, 54);
		panelCaution.add(refuseButton);

		preparedButton = new JxToggleButton();
		preparedButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				doSave(3, PanelMedicationCaution.this.currentMedication);
			}
		});
		...
		medicineName.setText(frameMedicationDesktop.getCurrentMedication()
				.getMedication()
				+ "");
		medicineName.setBounds(23, 21, 269, 26);
		panelCaution.add(medicineName);
		updateButtonsAccordingToStatus(this.currentMedication.getStatus());
		doLoad();
	}
	...
	// change here according to status to make buttons visible / invisible..
	/**
	 * 1-given 2- refused 3- prepared
	 */

	private void updateButtonsAccordingToStatus(int status) {
		if (status == 1) {
	...
	public void doSave(int status, MedicationView row) {
