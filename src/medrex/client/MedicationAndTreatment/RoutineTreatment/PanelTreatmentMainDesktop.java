...
public class PanelTreatmentMainDesktop extends JPanel {
	...
	public PanelTreatmentMainDesktop(
			FrameMedicationDesktop frameMedicationDesktop, String floorType) {
		super();
		setLayout(new BorderLayout());
		setBackground(new Color(231, 239, 242));
		this.parent = frameMedicationDesktop;
		...
		panelResiInfo.setBounds(795, 72, 40, 395);
		...
		historyList.setBounds(173, 72, 616, 172);
		...
		historyList.setCellRenderer(historyList.new JxListCellRenderer() {
			...
			protected String getCenterValue(Object value) {
				if (value instanceof Object[]) {
					/*
					 * if (values[5] != null || values[5] != "" ) { return
					 * (String) values[5]; }
					 */
					return "";
				}
			...
			protected String getPreviousSlotValue(Object value) {
				...
				return super.getPreviousSlotValue(value);
			}
		});
		...
		chooseResidentLabel.setText("Treatment Cart (RT)");
		...
		currentList.setBounds(173, 288, 616, 179);
		...
		currentList.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent evt) {
				if (evt.getPropertyName().equalsIgnoreCase(
					...
					PanelTreatmentMainDesktop.this.parent.setCurrentIndex(8);
					PanelTreatmentMainDesktop.this.parent.reDrawTheFrame();
					// PanelResidentInformation panel = new
					// PanelResidentInformation();
					// panel.setSize(400,500);
					// panel.setPreferredSize(new Dimension(400,500));
					// SwingUtils.wrapInDialog(panel).setVisible(true);
				}
			}
		});
		...
		/*
		 * final JxButton nextButton = new JxButton();
		 * nextButton.addActionListener(new ActionListener() { public void
		 * actionPerformed(final ActionEvent e) {
		 * Global.frameMedicationDashboard.setCurrentIndex(3);
		 * Global.frameMedicationDashboard.reDrawTheFrame(); } });
		 * nextButton.setFont(new Font("Dailog", Font.BOLD, 14));
		 * nextButton.setArc(0.4f);
		 * nextButton.setForeground(ColorConstants.GradientDark);
		 * nextButton.setBackground(Color.WHITE); nextButton.setText("NEXT");
		 * nextButton.setBounds(67, 31, 115, 35); defaultPanel.add(nextButton);
		 */
		...
		panel_1.setBounds(173, 250, 616, 41);
		defaultPanel.add(panel_1);

		final JRadioButton rbSortTime = new JRadioButton();
		rbSortTime.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				doSort("SORT_TIME", PanelTreatmentMainDesktop.this.elements);
			}
		});
		rbSortTime.setBackground(Color.WHITE);
		buttonGroup.add(rbSortTime);
		rbSortTime.setFont(new Font("", Font.BOLD, 14));
		rbSortTime.setOpaque(false);
		rbSortTime.setHorizontalAlignment(SwingConstants.LEADING);
		rbSortTime.setText("Sort By Time");
		rbSortTime.setForeground(ColorConstants.MED_COLOR);
		rbSortTime.setBounds(435, 0, 159, 24);
		panel_1.add(rbSortTime);

		final JRadioButton rbSortRoom = new JRadioButton();
		rbSortRoom.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				doSort("SORT_ROOM", PanelTreatmentMainDesktop.this.elements);
			}
		});
		...
		rbSortRoom.setBounds(236, 0, 185, 24);
		...
		rbSortLastName.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				doSort("SORT_LAST_NAME",
						PanelTreatmentMainDesktop.this.elements);
			}
		});
		...
		panel_2.setBounds(173, 473, 616, 53);
		...
		try {
			if (txtChooseResident.getText().length() > 0) {
				if (!txtChooseResident.getText().equalsIgnoreCase("")) {
					...
					for (Object obj : this.elements) {
						...
						if (((String) residentInfo[0]).toLowerCase()
			...
			if (data.length > 0) {
				for (Object obj : data) {
					...
					if (item[0] != null && selectedItem != null) {
						if (((String) item[0])
								.equalsIgnoreCase((String) selectedItem[0])) {
	...
	private void doSort(String sortType, List elements) {
		...
		if ("SORT_TIME".equalsIgnoreCase(sortType)) {
			...
			comparator = new Comparator() {
				...
				public int compare(Object o1, Object o2) {
					...
					if (o1 != null && o2 != null) {
						Object[] objArr1 = (Object[]) o1;
						Object[] objArr2 = (Object[]) o2;
						return ((String) objArr1[1])
								.compareToIgnoreCase((String) objArr2[1]);
					}
					return obj;
				}
	...
	private void showCurrentSlotList() {
		...
		while (it.hasNext()) {
			...
			String currentSlot = null, nextSlot = null, previousSlot = null;
			Integer prevSlotStatus = 0;

			if (o[1] != null) {
				obj[1] = TIME_FORMATTER.format(o[1]).toString();
				currentSlot = (String) obj[1];
			}
			if (o[2] != null) {
				obj[2] = TIME_FORMATTER.format(o[2]).toString();
				nextSlot = (String) obj[2];
			}
			if (o[10] != null) {
				obj[6] = o[10];
				previousSlot = (o[10]).toString();
				Date date = null;
				try {
				...
				previousSlot = TIME_FORMATTER.format(date).toString();
			}
			if (o[11] != null) {
				obj[7] = Integer.parseInt(o[11].toString());
				prevSlotStatus = (Integer) obj[7];
			}
			obj[3] = getHighlightedTimeSelection(currentSlot, nextSlot);
			obj[6] = getBlinkingTimeSelection(previousSlot, prevSlotStatus);
			obj[4] = ((Integer) o[3]).intValue();
			if (o[8] != null) {
				obj[5] = o[8].toString();
				previousSlot = (String) obj[5];

			}
			// System.out.println("Room :"+str3);
			newElements.add(obj);
			elements.add(obj);
		}
	...
	private Object getBlinkingTimeSelection(String previousSlot,
			Integer prevSlotStatus) {

		String tmp = "<html><body><blink>";
		if (previousSlot != null && prevSlotStatus != 0) {
			tmp += "<font color='#8D38C9'>" + previousSlot.toString()
					+ "</font>";
		}
		if (previousSlot != null && prevSlotStatus == 0) {
			tmp += "<font color='#8D38C9'>" + previousSlot.toString()
	...
	// SwingUtils.wrapInDialog(new PanelMedicationMainDesktop(new
	// FrameMedicationDesktop())).setVisible(true);
	// }
