...
public class PanelMedicationPRNRecordForm extends JPanel {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2780265070075340916L;
	...
	PanelMedicationPRNRecordFormRow panelPRNRows[];

	/**
	 * Create the panel
	 */
	public PanelMedicationPRNRecordForm() {
		super();
		setBorder(new LineBorder(new Color(34, 139, 34), 1, false));
		setSize(1200, 950);
		Global.panelMedicationPRNRecordForm = this;
		...
		prnMedicationsLabel.setBounds(200, 0, 120, 30);
		...
		prnMedicationsLabel.setText("PRN Medications");
		...
		dPrnMedicationsLabel
				.setText("D. PRN Medications: Reason given results must be noted");
		...
		panelPRNRows = new PanelMedicationPRNRecordFormRow[7];

		for (i = 0; i < 7; i++) {
			panelPRNRows[i] = new PanelMedicationPRNRecordFormRow(i + 1);
	...
	public void doSave() {
		MedicationPRNRecordForm mprf = null;

		if (Global.currentMedicationPRNRecordFormKey == 0) {
			mprf = new MedicationPRNRecordForm();
			mprf.setSerial(Global.currentMedicationPRNRecordFormKey);
			mprf.setResidentId(Global.currentResidenceKey);
			try {
				Global.currentMedicationPRNRecordFormKey = MedrexClientManager
						.getInstance().insertOrUpdateMedicationPRNRecordForm(
								mprf);
			} catch (Exception e) {
