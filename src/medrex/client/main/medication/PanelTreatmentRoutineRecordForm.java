...
public class PanelTreatmentRoutineRecordForm extends JPanel {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1331966641401509365L;
	...
	PanelTreatmentRoutineRecordFormRow panelRoutineRows[];

	/**
	 * Create the panel
	 */
	public PanelTreatmentRoutineRecordForm() {
		super();
		setBorder(new LineBorder(new Color(72, 61, 139), 1, false));
		setSize(1200, 950);
		Global.panelTreatmentRoutineRecordForm = this;
		...
		prnTreatmentsLabel.setText("Routine TREATMENTS");
		...
		panelRoutineRows = new PanelTreatmentRoutineRecordFormRow[7];

		for (i = 0; i < 7; i++) {
			panelRoutineRows[i] = new PanelTreatmentRoutineRecordFormRow(i + 1);
	...
	public void doSave() {
		TreatmentRoutineRecordForm mprf = null;

		if (Global.currentTreatmentRoutineRecordFormKey == 0) {
			mprf = new TreatmentRoutineRecordForm();
			mprf.setSerial(Global.currentTreatmentRoutineRecordFormKey);
			mprf.setResidentId(Global.currentResidenceKey);
			try {
				Global.currentTreatmentRoutineRecordFormKey = MedrexClientManager
						.getInstance()
						.insertOrUpdateTreatmentRoutineRecordForm(mprf);
			} catch (Exception e) {
