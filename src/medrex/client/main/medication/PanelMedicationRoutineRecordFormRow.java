...
public class PanelMedicationRoutineRecordFormRow extends JPanel {

	/**
	 * 
	 */
	private static final long serialVersionUID = 6270481923486781304L;
	...
	private JTextArea txtRoutineMedications;
	...
	PanelMedicationRoutineRecordFormCol panelRoutineCols[];
	/**
	 * Create the panel
	 */
	...
	public PanelMedicationRoutineRecordFormRow(int num) {
		super();
		...
		txtRoutineMedications = new JTextArea();
		txtRoutineMedications.setLineWrap(true);
		txtRoutineMedications.setBounds(10, 10, 182, 100);
		panel_6.add(txtRoutineMedications);
		...
		panelRoutineCols = new PanelMedicationRoutineRecordFormCol[31];

		for (i = 0; i < 31; i++) {
			panelRoutineCols[i] = new PanelMedicationRoutineRecordFormCol(num,
					i);
	...
	public void updateData() {
		if (Global.currentMedicationRoutineRecordFormKey == 0) {
			key = 0;
			txtRoutineMedications.setText("");
			txtRxNumber.setText("");
			dateChooser.setDate(null);

		} else {
			MedicationRoutineRecordFormRow mprfr = null;
			try {
				mprfr = MedrexClientManager.getInstance()
						.getMedicationRoutineRecordFormRow(
			...
			txtRoutineMedications.setText(mprfr.getMedicationRoutine());
	...
	public void doSave() {
		MedicationRoutineRecordFormRow mprfr = new MedicationRoutineRecordFormRow();

		mprfr.setSerial(key);
		mprfr
				.setMedicationRoutineRecordFormSerial(Global.currentMedicationRoutineRecordFormKey);
		mprfr.setMedicationRoutineRecordFormRowSerial(serial);

		mprfr.setOrderDate(dateChooser.getDate());
		mprfr.setRxNumber(txtRxNumber.getText());
		mprfr.setMedicationRoutine(txtRoutineMedications.getText());

		try {
			/* Global.currentMedicationRoutineRecordFormKey = */MedrexClientManager
					.getInstance()
					.insertOrUpdateMedicationRoutineRecordFormRow(mprfr);
		} catch (Exception e) {
